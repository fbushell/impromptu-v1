Y.use('node', function (Y) {
	...
	Y.namespace('Template').Site = Singleton.create({
		...
		init: function() {
			...
			//this.getVariables();
			/*
				Inject the content for the show
				on scroll script. We'll initiate
				that script later.
			*/
		...
		bindUI: function () {
			...
			this.mutationObserver(Y.one('#siteWrapper').getDOMNode(), { childList: true, subtree: true }, this.mutationCallback);

			Y.one(window).on('resize', this.getVariables, this);
			...
			/*
				If the index page has a bunch of images
				lazyload them. Otherwise, just let them
				load normally.
			*/
			...
			//this.scrollNav();
			this.altSections(Y.all('.index-section.no-main-image + .index-section.no-main-image'));
		...
		// 		// re-wrote this in vanilla javascript in site.region so it executes immediately
